...
class BallMarker(object):
    ...
    def setPose(self, position):
        """
        Set the position (3x1 NumPy matrix) for the ball and publish it

        """
        if (len(position)>=3):
            self.marker.pose.position.x = position[0][0]
            self.marker.pose.position.y = position[1][0]
            self.marker.pose.position.z = position[2][0]
            self.publish()
        else:
            rospy.loginfo('BallMarker: Not enough position elements.')

    def setPoseFromLink(self, robot, link):
        """
        Set the position specifying the link name or id and publish it (this can be
        mainly used to test the model in RViz)

        """
        position = robot.linkPosition(link)
